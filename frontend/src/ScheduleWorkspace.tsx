...
function ManualCoursePanel({ catalog, plan }: { catalog: Course[]; plan: SchedulePlan }) {
  ...
  const addCourse = async () => {
    setMessage("");
    if (!selectedCourse) return setMessage("請先搜尋並選擇要加入的課程。");
    if (plan.entries.some((entry) => entry.courseId === selectedCourse.course_id)) return setMessage("這門課已經在目前課表中。");
    let meetings = selectedCourse.meetings;
    if (customTimeActive) {
      const sections = parseManualSections(customSections);
      if (!sections.length) return setMessage("請輸入有效節次，例如 D5,D6 或 DN。");
      meetings = [{ weekday: customWeekday, sections, room: null, week_pattern: "A" }];
    }
    if (!meetings.length) return setMessage("這門課沒有可用的上課時間，請使用指定時間加入。");
    ...
    const fixedConflict = meetingsConflict(meetings, (plan.fixedEntries ?? []).flatMap((entry) => entry.meetings));
    if (courseConflict.conflict || fixedConflict.conflict || courseConflict.uncertain || fixedConflict.uncertain) {
      const reason = courseConflict.conflict || fixedConflict.conflict
        ? "這門課與目前課表或固定時段衝堂。" : "這門課的週次資料不完整，可能與目前課表衝堂。";
      if (!window.confirm(`${reason}\n仍要加入課表嗎？`)) return;
      setMessage(`${reason}你仍選擇加入，請再確認課表。`);
    }
    const entry: ScheduleEntry = {
      courseId: selectedCourse.course_id,
      locked: false,
      ...(customTimeActive ? { meetingsOverride: meetings } : {}),
    };
    await putRecord("schedulePlans", { ...plan, entries: [...plan.entries, entry], updatedAt: new Date().toISOString() });
    setSelectedCourseId("");
    setQuery("");
    setUseCustomTime(false);
    setMessage(`已加入「${selectedCourse.name_zh}」。`);
  };
  ...
  return <section className="card schedule-add-card">
    ...
    <button className="primary" type="button" onClick={() => void addCourse()} disabled={!selectedCourse}>加入「{plan.name}」</button>
  </section>;
}
...
function CourseDetails({ block, catalog, scheduledCourses, plan, onClose }: { block: ScheduleBlock; catalog: Course[]; scheduledCourses: Course[]; plan: SchedulePlan; onClose: () => void }) {
  ...
  return <div className="schedule-dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <aside className="schedule-dialog" role="dialog" aria-modal="true" aria-labelledby="schedule-dialog-title">
      <div className="schedule-dialog-heading"><div><div className="eyebrow">{block.source === "course" ? "課程詳細資訊" : "固定時段"}</div><h2 id="schedule-dialog-title">{scheduledCourse?.name_zh ?? fixedEntry?.name ?? block.name}</h2></div><button ref={closeRef} className="dialog-close" onClick={onClose} aria-label="關閉課程詳細資訊">×</button></div>
...
export function ScheduleWorkspace({ catalog, plans, active, selectPlan }: { catalog: Course[]; plans: SchedulePlan[]; active?: SchedulePlan; selectPlan: (planId: string) => Promise<void> }) {
  ...
  const createPlan = async () => {
    const name = window.prompt("課表方案名稱", `方案 ${plans.length + 1}`)?.trim();
    if (!name) return;
  ...
  const renamePlan = async () => {
    if (!active) return;
    const name = window.prompt("重新命名課表方案", active.name)?.trim();
    if (name && name !== active.name) await putRecord("schedulePlans", { ...active, name, updatedAt: new Date().toISOString() });
  };
  ...
  return <section className="page"><div className="page-heading"><div><div className="eyebrow">安排多個選課方案</div><h1>我的課表</h1></div><button className="primary" onClick={() => void createPlan()}>＋ 新增方案</button></div>
    <div className="plan-tabs">{plans.map((plan) => <button className={plan.id === active?.id ? "active" : ""} onClick={() => void selectPlan(plan.id)} key={plan.id}>{plan.name}</button>)}</div>
    {!active ? <section className="empty-state"><h1>先建立一個課表方案</h1><p>建立課表後，你可以加入課程並檢查衝堂。</p><button className="primary" onClick={() => void createPlan()}>建立第一個方案</button></section> : <>
      <div className="schedule-plan-actions"><strong>目前方案：{active.name}</strong><button onClick={() => void renamePlan()}>重新命名</button><button onClick={() => void duplicatePlan()}>建立副本</button><button onClick={printSchedule}>列印／另存 PDF</button></div>
      <ManualCoursePanel catalog={catalog} plan={active} />
      <div className="schedule-summary"><strong>{courses.length} 門課</strong><span>{credits} 學分</span>{fixedEntries.length > 0 && <span>{fixedEntries.length} 個固定時段</span>}{conflictCount > 0 && <span className="schedule-conflict-summary">⚠ {conflictCount} 門課衝堂</span>}{unplacedCourses.length > 0 && <span>待安排 {unplacedCourses.length} 門</span>}</div>
      ...
      <div className="timetable" aria-label={`${active.name}課表`}><div className="schedule-grid" style={gridStyle} role="grid"><div className="schedule-corner" role="columnheader">節次</div>{visibleDays.map((day, dayIndex) => <div className="schedule-day-header" role="columnheader" key={day} style={{ gridColumn: dayIndex + 2, gridRow: 1 }}>星期{weekdays[day - 1]}</div>)}{visibleSections.map((section, sectionIndex) => <div className={`schedule-section-label ${EXTENDED_SCHEDULE_SECTIONS.includes(section as typeof EXTENDED_SCHEDULE_SECTIONS[number]) ? "extended" : ""}`} role="rowheader" key={section} style={{ gridColumn: 1, gridRow: sectionIndex + 2 }}>{section}</div>)}{visibleSections.flatMap((section, sectionIndex) => visibleDays.map((day, dayIndex) => <div className="schedule-cell" role="gridcell" aria-label={`星期${weekdays[day - 1]} ${section}`} key={`${day}-${section}`} style={{ gridColumn: dayIndex + 2, gridRow: sectionIndex + 2 }} />))}{blocks.map((block) => {
        ...
        return <button type="button" className={`class-block ${block.source === "fixed" ? "fixed" : ""} ${block.conflict ? "conflict" : ""}`} style={blockStyle} key={block.id} onClick={(event) => openDetails(block, event.currentTarget)} aria-label={`${block.name}，星期${weekdays[block.weekday - 1]} ${block.sections.join("到")}${block.conflict ? "，有衝堂" : ""}`}><strong>{block.name}</strong><small>{block.teacher}</small>{block.room && <small>📍 {block.room}</small>}<span className="class-block-tags">{weekPatternLabel(block.weekPattern) && <em>{weekPatternLabel(block.weekPattern)}</em>}{block.conflict && <em className="conflict-tag">⚠ 衝堂</em>}</span></button>;
      })}</div><div className="mobile-schedule-list">{!mobileBlocks.length && <p className="muted">星期{weekdays[mobileDay - 1]}目前沒有課程。</p>}{mobileBlocks.map((block) => <button key={block.id} className={`mobile-schedule-block ${block.source === "fixed" ? "fixed" : ""} ${block.conflict ? "conflict" : ""}`} onClick={(event) => openDetails(block, event.currentTarget)}><span><strong>{block.sections.join("–")}　{block.name}</strong><small>{block.teacher}{block.room ? ` · ${block.room}` : ""}</small></span><span>{weekPatternLabel(block.weekPattern)}{block.conflict ? " ⚠" : ""}</span></button>)}</div></div>
