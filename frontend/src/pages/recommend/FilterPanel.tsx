...
const MOBILE_SEARCH_FIRST_THRESHOLD = 12;
...
function SearchableMultiSelect({ label, options, selected, onChange, suggestedValue, searchFirst = false }: {
    label: string;
    options: FacetOption[];
    selected: string[];
...
export function FilterPanel(props: FilterPanelProps) {
    ...
    const mobileSearchFirst = mode === "modal" && !isDesktop;
    const searchFirstFor = (options: FacetOption[]) => mobileSearchFirst && options.length > MOBILE_SEARCH_FIRST_THRESHOLD;
    ...
    const literacyOptions = relationBy(["literacy"]);
    const coreCompetencyOptions = relationBy(["core_knowledge", "core_skills_attitudes"]);
    const specialIssueOptions = relationBy(["special_issues"]);
